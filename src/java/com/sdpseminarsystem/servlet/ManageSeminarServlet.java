...
public class ManageSeminarServlet extends HttpServlet {
                ...
                if(request.getParameter("venue") == null){
                    try {
                            ...
                            for (User user : allUsers) {
                                if  (user.getUserTypeFlag().equals('h') && user.getUserId().equals(request.getParameter("host").substring(0, 8))){
                                    out.write("<option selected>" + user.getUserId() + " " + user.getUserFirstName() + " " + user.getUserLastName() + "</option>");
                                } else if(user.getUserTypeFlag().equals('h')){
                                    out.write("<option>" + user.getUserId() + " " + user.getUserFirstName() + " " + user.getUserLastName() + "</option>");
                                }
                            }
    ...
    private void updateSeminar(HttpServletRequest request, HttpServletResponse response)
		...
		DAOFactory.getInstanceOfSeminarDAO().update(seminar);
                ...
                List<Speaker> allSpeakers = DAOFactory.getInstanceOfSpeakerDAO().findAllBySeminar(Integer.parseInt(request.getParameter("seminarId")));
